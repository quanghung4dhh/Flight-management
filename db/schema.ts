...
export const userProfiles = mysqlTable("user_profiles", {
  id: serial("id").primaryKey(),
  userId: bigint("userId", { mode: "number", unsigned: true }).notNull().references(() => users.id).unique(),
...
export const routes = mysqlTable("routes", {
  id: serial("id").primaryKey(),
  departureAirportId: bigint("departure_airport_id", { mode: "number", unsigned: true }).notNull().references(() => airports.id),
  arrivalAirportId: bigint("arrival_airport_id", { mode: "number", unsigned: true }).notNull().references(() => airports.id),
...
export const maintenanceLogs = mysqlTable("maintenance_logs", {
  id: serial("id").primaryKey(),
  aircraftId: bigint("aircraft_id", { mode: "number", unsigned: true }).notNull().references(() => aircraft.id),
  maintenanceType: mysqlEnum("maintenance_type", ["routine", "repair", "inspection"]).notNull(),
  ...
  index("maintenance_aircraft_idx").on(table.aircraftId),
]);
...
export const flights = mysqlTable("flights", {
  ...
  routeId: bigint("route_id", { mode: "number", unsigned: true }).notNull().references(() => routes.id),
  aircraftId: bigint("aircraft_id", { mode: "number", unsigned: true }).notNull().references(() => aircraft.id),
  ...
  status: mysqlEnum("status", ["scheduled", "boarding", "departed", "arrived", "delayed", "cancelled"]).default("scheduled").notNull(),
  ...
  premiumPrice: decimal("premium_price", { precision: 10, scale: 2 }).notNull().default("0"),
  businessPrice: decimal("business_price", { precision: 10, scale: 2 }).notNull().default("0"),
...
export const seats = mysqlTable("seats", {
  ...
  seatClass: mysqlEnum("seat_class", ["economy", "premium", "business"]).notNull(),
  seatType: mysqlEnum("seat_type", ["window", "aisle", "middle", "exit_row"]).notNull(),
  extraPrice: decimal("extra_price", { precision: 10, scale: 2 }).notNull().default("0"),
  seatMapRow: int("seat_map_row").notNull(),
  seatMapCol: int("seat_map_col").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => [
  uniqueIndex("seat_aircraft_number_idx").on(table.aircraftId, table.seatNumber),
]);
...
// 9. FLIGHT SEATS
// ==========================================
export const flightSeats = mysqlTable("flight_seats", {
  id: serial("id").primaryKey(),
  flightId: bigint("flight_id", { mode: "number", unsigned: true }).notNull().references(() => flights.id),
  seatId: bigint("seat_id", { mode: "number", unsigned: true }).notNull().references(() => seats.id),
  status: mysqlEnum("status", ["available", "occupied", "blocked"]).default("available").notNull(),
  bookedBy: bigint("booked_by", { mode: "number", unsigned: true }).references(() => users.id),
...
export const bookings = mysqlTable("bookings", {
  ...
  flightId: bigint("flight_id", { mode: "number", unsigned: true }).notNull().references(() => flights.id),
  tripType: mysqlEnum("trip_type", ["one_way", "round_trip"]).default("one_way").notNull(),
  returnFlightId: bigint("return_flight_id", { mode: "number", unsigned: true }).references(() => flights.id),
...
export const tickets = mysqlTable("tickets", {
  id: serial("id").primaryKey(),
  bookingId: bigint("booking_id", { mode: "number", unsigned: true }).notNull().references(() => bookings.id),
  ...
  seatId: bigint("seat_id", { mode: "number", unsigned: true }).references(() => seats.id),
  flightSeatId: bigint("flight_seat_id", { mode: "number", unsigned: true }).references(() => flightSeats.id),
  status: mysqlEnum("status", ["active", "used", "cancelled", "refunded"]).default("active").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => [
...
export const payments = mysqlTable("payments", {
  ...
  method: mysqlEnum("method", ["credit_card", "debit_card", "momo", "zalopay", "qr_code"]).notNull(),
...
export const crewMembers = mysqlTable("crew_members", {
  id: serial("id").primaryKey(),
  userId: bigint("user_id", { mode: "number", unsigned: true }).notNull().references(() => users.id).unique(),
  employeeCode: varchar("employee_code", { length: 50 }).notNull().unique(),
  position: mysqlEnum("position", ["captain", "first_officer", "flight_attendant"]).notNull(),
...
// 14. FLIGHT CREW
// ==========================================
export const flightCrew = mysqlTable("flight_crew", {
  id: serial("id").primaryKey(),
  flightId: bigint("flight_id", { mode: "number", unsigned: true }).notNull().references(() => flights.id),
  crewMemberId: bigint("crew_member_id", { mode: "number", unsigned: true }).notNull().references(() => crewMembers.id),
  roleOnFlight: mysqlEnum("role_on_flight", ["captain", "first_officer", "lead_attendant", "attendant"]).notNull(),
...
export const notifications = mysqlTable("notifications", {
  id: serial("id").primaryKey(),
  userId: bigint("user_id", { mode: "number", unsigned: true }).notNull().references(() => users.id),
  type: mysqlEnum("type", ["booking_confirm", "flight_delay", "flight_cancel", "reminder", "promo"]).notNull(),
