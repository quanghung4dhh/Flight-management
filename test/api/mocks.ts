...
export function createMockDb(overrides: any = {}) {
  ...
  return {
    ...
    insert: vi.fn(() => ({ values: vi.fn(() => Promise.resolve(overrides.insert ?? [])) })),
    update: vi.fn(() => ({ set: vi.fn(() => ({ where: vi.fn(() => Promise.resolve(overrides.update ?? [])) })) })),
    delete: vi.fn(() => ({ where: vi.fn(() => Promise.resolve(overrides.delete ?? [])) })),
